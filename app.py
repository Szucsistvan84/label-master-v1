    ...
    for page in reader.pages:
        ...
        for line in lines:
            ...
            if match:
                ...
                # Kapukód és egyéb infó keresése
                elif any(x in line.lower() for x in ['kcs', 'kód', 'kk:', 'kapukód', 'kulcs', 'itthon']):
                ...
                # Ha még nincs cím és nem rendelés, akkor a név vagy cím folytatása
...
if uploaded_file and futar_nev and futar_tel:
    try:
        ...
        if not data:
            st.warning("Nem találtam adatokat a PDF-ben.")
        else:
            st.success(f"Siker! {len(data)} etikett kész.")
            ...
            for i, item in enumerate(data):
                ...
                if item['kapukod']:
                    ...
                    # Több sorba törjük az infót, ha hosszú
                    info = item['kapukod'].strip()[:45]
                    c.drawString(x+10, y+cell_h-42, f"INFO: {info}")
                    c.setFillColorRGB(0, 0, 0)
                
                c.setFont("Helvetica-Bold", 12)
                rendeles = item['rendeles'].strip()[:30]
                c.drawString(x+10, y+35, f"{rendeles}")
