...
def parse_interfood_pro(pdf_file):
    ...
    # Szigorú pénz minta: kezeli a mínuszt és az ezres szóközt is
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            for y in sorted(lines.keys()):
                ...
                # Eredeti, jól működő blokk-felosztás a névhez és címhez
                b3 = " ".join([w['text'] for w in line_words if 150 <= w['x0'] < 355])
                b4 = " ".join([w['text'] for w in line_words if 355 <= w['x0'] < 490])
                
                # NÉV: Marad a régi bevált módszer
                clean_name = re.sub(r'[^a-zA-ZáéíóöőúüűÁÉÍÓÖŐÚÜŰ \-]', '', b4).strip()
                
                # TELEFON ÉS PÉNZ: A teljes sorban keressük, mert gyakran egymás mellett/alatt vannak
                tel_m = re.search(phone_pat, text_ws.replace(" ", ""))
                
                # ÚJ PÉNZKERESŐ: Ha van "Ft" a sorban, azt szedjük ki
