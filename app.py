...
# --- 2. ONLINE ÉTLAP (HIBATŰRŐ VERZIÓ) ---
def get_live_menu_data(meta_list):
    ...
    for m in meta_list:
        if m.get('year') and m.get('week'):
            year, week = m['year'], m['week']
            break
    
    # Ha nincs meg az év/hét, meg sem próbáljuk, így elkerüljük a 400-as hibát
    ...
    try:
        ...
        pass # Bármilyen hiba esetén simán továbblépünk
    return menu_map
...
# --- 4. PDF GENERÁTOROK (MENETTERV, RAKLISTA) ---
def create_manifest_pdf(df, fn, meta_list):
    ...
    for p_idx in range(pages):
        ...
        t = Table(data, colWidths=[8*mm, 72*mm, 8*mm, 22*mm, 72*mm, 8*mm])
...
def create_raklista_pdf(df, jarat, menu):
    ...
    for i in range(0, len(data), rows_p_p):
        p.setFont(f_bold, 12); p.drawString(10*mm, h-15*mm, f"RAKLISTA - Járat: {jarat}")
        ...
        t.wrapOn(p, 10*mm, 30*mm); t.drawOn(p, 10*mm, h - 30*mm - (len(subset)*6.5*mm))
        if i + rows_p_p >= len(data):
            curr_y = h - 45*mm - (len(subset)*6.5*mm)
...
with st.sidebar:
    ...
    if pdf_files and st.button("🚀 PDF Feldolgozás"):
        raw, meta = [], []
        for f in pdf_files:
        ...
        if raw:
            ...
            st.rerun()
    st.divider()
    csv_file = st.file_uploader("2. CSV visszatöltés", type=['csv'])
    if csv_file:
...
if st.session_state.mdf is not None:
    ...
    c3.download_button("📊 CSV EXPORT", edited_df.to_csv(index=False).encode('utf-8-sig'), "napi_adatok.csv")
