...
# --- PDF PARSER ÉS MERGE (A korábbi stabil verziók alapján) ---
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            for i, y in enumerate(sorted_y):
                ...
                if v_o:
                    rows.append({"Prefix": prefix, "ID": uid, "Ügyintéző": clean_name, "Cím": address, "Telefon": tel_m.group(0) if tel_m else "", "Rendelés": ", ".join(v_o), "Pénz": money_val, "Összesen": sq})
    return rows
...
if 'live_menu' not in st.session_state: st.session_state.live_menu = get_live_menu()
...
with st.sidebar:
    ...
    with st.expander("Vészhelyzeti Étlap Frissítés"):
        st.write("Nyisd meg az Interfoodot, Ctrl+A, Ctrl+C, majd ide Ctrl+V:")
        pasted = st.text_area("Beillesztés", height=100)
        if st.button("Frissítés most"):
            st.session_state.live_menu = get_live_menu(manual_text=pasted)
            st.rerun()
