...
st.set_page_config(page_title="Interfood Logisztika v203.4", layout="wide")
...
# --- PDF PARSER (Prefix: H, K, S, C, P, Z kezelés) ---
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            for y in sorted(lines.keys()):
                ...
                # Prefixek keresése: H-fő, K-edd, S-zerda, C-sütörtök, P-éntek, Z-(szombat)
                ...
                # Koordináta alapú név és cím kinyerés
...
def merge_data_flexible(raw_rows):
    ...
    for uid, group in df.groupby("ID", sort=False):
        # Csak Péntek (P) és Szombat (Z) esetén vonunk össze
        if any(p in ['P', 'Z'] for p in group['Prefix']):
            ...
            # Hétköznap (H, K, S, C) nincs összevonás
            for _, row in group.iterrows():
                merged.append(row.to_dict())
    return merged

# --- ETIKETT GENERÁLÁS (21/lap) ---
def create_label_pdf(df, fn, ft):
    ...
    for i in range(total_slots):
        ...
        if i < total_labels:
            ...
            p.setFont(f_bold, 9); p.drawCentredString(x+lw/2, y+25*mm, "Üres etikett")
            
    p.save(); buf.seek(0)
    return buf

# --- MENETTERV GENERÁLÁS (Max 22 sor/oldal) ---
...
st.title("🏷️ Interfood Logisztika v203.4")
...
if st.session_state.get('mdf') is not None:
    st.table(st.session_state.mdf.head(10)) # Előnézet
    c1, c2 = st.columns(2)
    with c1:
        num_pages = math.ceil(len(st.session_state.mdf) / 21)
        st.info(f"🖨️ Nyomtatás: **{num_pages} lap** etikett szükséges.")
        st.warning("⚠️ A lapokat **ETIKETTEL LEFELÉ** helyezd be!")
        if st.button("📥 ETIKETTEK LETÖLTÉSE"):
            pdf = create_label_pdf(st.session_state.mdf, fn, ft)
            st.download_button("Mentés", pdf, "etikettek.pdf")
    with c2:
        if st.button("📋 MENETTERV LETÖLTÉSE"):
            pdf = create_manifest_pdf(st.session_state.mdf, fn)
            st.download_button("Mentés", pdf, "menetterv.pdf")
