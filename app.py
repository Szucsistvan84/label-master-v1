                ...
                if u_code_m:
                    ...
                    if v_o:
                        rows.append({
                            ...
                            "Pénz_Int": custom_round(raw_money), "Kapukód": gate_code
                        })
    return rows
...
def create_label_pdf(df, fn, ft):
    ...
    for i in range(math.ceil(len(df)/21)*21):
        ...
        if i < len(df):
            ...
            p.setFont(f_reg, 5.5); p.drawCentredString(x+lw/2, y+5*mm, "*a kedvezmény telefonon leadott rendelésekre érvényesíthető")
...
if up_files and st.button("📊 FELDOLGOZÁS"):
    ...
    if raw:
        ...
        cols = ['Sorrend', 'ID', 'Ügyintéző', 'Kapukód', 'Cím', 'Telefon', 'Rendelés', 'Összesen', 'Pénz']
