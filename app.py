...
if st.session_state.nezettipus == "asztali":
    ...
    # ---- BIZTONSÁGI SZŰRŐ ÜRES ADATOKRA (Golyóálló verzió) ----
    ...
    # 🟢 A NAGY MENTŐÖV VÉDŐHÁLÓ: Csak akkor rajzolunk ki bármit, ha VALÓBAN van adat feldolgozva!
    if "mdf" in st.session_state and st.session_state.mdf is not None and not st.session_state.mdf.empty:
        ...
        # PC FELÜLET LEGALJA - MOBIL INDÍTÓ EGYSÉG QR-KÓDDAL
        st.markdown("---")
    ...
    st.markdown(f"<p style='text-align: center; color: #6B7280; font-size:14px;'>Futár: <b>{st.session_state.get('user_nev', 'Teszt Futár')}</b> | Járat(ok): {', '.join(st.session_state.get('aktiv_jaratok', ['4002']))}</p>", unsafe_allow_html=True)
    ...
    # Itt hívjuk meg a fájl végén lévő 3-fázisú mobil felületedet
    futar_mobil_felulet()
