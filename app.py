            ...
            for i in range(len(df)):
                ...
                if col_a and col_a != 'nan' and " - " in col_a:
                    ...
                    if name_on_day and name_on_day != 'nan' and len(name_on_day) > 2:
                        ...
                        menu_map[code.strip()] = {'nev': name_on_day[:65], 'ar': int(p_str or 0), 'kategoria': cat.strip(), 'excel_order': i}
    except: pass
    return menu_map
...
# --- 4. RAKLISTA ÉS UI (Változatlan, de fontos részek) ---
...
with st.sidebar:
    ...
    st.divider()
    prev_csv = st.file_uploader("Előző napi CSV betöltése", type="csv")
    if prev_csv:
        pdf_c = pd.read_csv(prev_csv)
        st.session_state.weights = dict(zip(pdf_c['ID'].astype(str), pdf_c['Sorrend'].astype(float)))
        if 'Megjegyzés' in pdf_c.columns: st.session_state.notes = dict(zip(pdf_c['ID'].astype(str), pdf_c['Megjegyzés'].fillna("")))
    ...
    if up_files and st.button("🚀 FELDOLGOZÁS"):
        ...
        df_merged = []
        for uid, group in pd.DataFrame(all_raw).groupby("ID", sort=False):
            ...
            df_merged.append(base)
        res = pd.DataFrame(df_merged)
...
if st.session_state.mdf is not None:
    ...
    with c1:
        if st.button("💾 SORREND MENTÉSE", use_container_width=True):
            ...
            st.rerun()
    with c2:
        st.download_button("📥 CSV EXPORT", edited.to_csv(index=False).encode('utf-8-sig'), "mentes.csv", use_container_width=True)
    with c3:
