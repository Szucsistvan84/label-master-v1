...
def parse_v86(df_raw):
    temp_storage = {} 
    order_list = [] 

    for idx, row in df_raw.iterrows():
        # Sor ellenőrzése, van-e benne ügyfélkód
        ...
        for i in range(0, len(row.values), 5):
            ...
            # 2. ÜGYINTÉZŐ NEVE (Szigorúan a 2-es index!)
            # Itt lakik a "színtiszta név"
            raw_name = str(row.values[i+2]).strip()
            
            # Szűrés: Ne legyen üres, ne legyen csak szám, ne legyen benne napkód
            if raw_name == "nan" or re.match(r'^\d+$', raw_name) or re.match(r'^[A-Z]-\d', raw_name):
                name = "Név hiba (üres/kód)"
            else:
                name = raw_name

            # 3. CÍM (Az 1-es cellából az irányítószámtól)
            ...
            # --- TÁROLÁS ---
            customer_key = (name, addr) # Ha a név és cím azonos, összevonunk
            ...
            else: temp_storage[customer_key]["P"] = current_rend_str # Egyéb napok alapértelmezetten P
...
# A PDF generátor marad a korábbi sorszámozott, letisztult designnál
def create_pdf_v86(df):
    pdf = FPDF()
...
st.title("Interfood v86 - A Névtisztító")
f = st.file_uploader("PDF feltöltése", type="pdf")
if f:
    with open("temp.pdf", "wb") as tp: tp.write(f.read())
    dfs = tabula.read_pdf("temp.pdf", pages='all', stream=True, guess=False)
    if dfs:
        final_df = parse_v86(pd.concat(dfs, ignore_index=True))
        st.dataframe(final_df)
        if not final_df.empty:
            st.download_button("💾 PDF LETÖLTÉSE", bytes(create_pdf_v86(final_df)), "etikettek_v86.pdf")
    os.remove("temp.pdf")
