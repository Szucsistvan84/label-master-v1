...
def parse_interfood_pdf(pdf_file):
    ...
    with pdfplumber.open(pdf_file) as pdf:
        # Dátum kinyerése a teljes első oldalról
        full_text = ""
...
def create_manifest_pdf(df, fn, meta):
    ...
    for p_idx in range(total_p):
        p.setFont(f_bold, 12)
        p.drawString(10*mm, h-10*mm, f"MENETTERV - {fn}")
        p.setFont(f_reg, 10)
        ...
        t.drawOn(p, 10*mm, h - 22*mm - t._height)
        p.showPage()
...
def create_raklista_pdf(df, fn):
    ...
    # Étlap adatok lekérése a sessionből (ha van)
    menu = st.session_state.get('live_menu', {})
    sum_rows = []
    total_val, total_items = 0, 0
    
    # Csak azokat listázzuk, amik benne vannak a rendelésben
