...
def render_mobil_aruatvetel(sheet):
    st.subheader("📦 Ömlesztett áruátvétel")
    
    # 1. JÁRAT ÉS FUTÁR AZONOSÍTÁS
    # Megpróbáljuk kinyerni az adatokat az Adatok munkalapból
    try:
        ...
        st.error(f"Nem sikerült beolvasni az Adatok munkalapot: {e}")
        return

    # Futár neve ( session_state-ből, ha van, különben teszt )
    futar_neve = st.session_state.get('user_nev', 'Te (Teszt Üzemmód)')
    
    # Járat választó a mobilra (nagy, könnyen nyomható legördülő)
    ...
    if not st.session_state.aruatvetel_folyamatban:
        ...
        if st.button("🚀 ÁRUÁTVÉTEL INDÍTÁSA", use_container_width=True, type="primary", key="futar_start_btn"):
            ...
            try:
                ...
                # Sor beszúrása: Dátum, Járat_ID, Futár, Áruátvétel_Start, Áruátvétel_End
                idok_sheet.append_row([mai_datum, valasztott_jarat, futar_neve, start_ido, ""])
                
                # Elmentjük a sor indexét a lezáráshoz
    ...
    # =========================================================================
    # ÁLLAPOT 2: AZ ÁRUÁTVÉTEL FOLYAMATBAN VAN (MUTATJUK A RAKLISTÁT)
    # =========================================================================
    else:
        st.warning(f"🔄 Áruátvétel folyamatban... (Járat: {valasztott_jarat})")
        
        # Szűrés a választott járatra
        ...
        if not df_jarat_cikkek.empty and 'Étel Neve' in df_jarat_cikkek.columns:
            # Összesítjük az ételeket darabszámra a járaton belüli darabszám (vagy sorok száma) alapján
            # Ha van konkrét 'Mennyiség' oszlopod, cseréld .size()-t .sum()-ra
            ...
            # Nagy, jól olvasható mobil listanézet checkboxokkal, hogy ki tudja pipálni magának pakolás közben
        ...
        # HIBABEJELENTÉS ÁRUÁTVÉTEL KÖZBEN
            ...
            if st.button("⚠️ HIBA BEKÜLDÉSE AZ ADMINNAK", use_container_width=True, key="mob_hiba_submit"):
                if hiba_etel != "":
                    try:
                        ...
                        # Sorrend: Időbélyeg, Járat_ID, Ügyfél Neve, Cikkszám, Étel Neve, Mennyiség, Összérték, Hiba Típusa, Megjegyzés, Futár, Admin_Státusz
        ...
        if st.button("🏁 ÁRUÁTVÉTEL LEZÁRÁSA & INDULÁS", use_container_width=True, type="secondary", key="futar_end_btn"):
            ...
            try:
                ...
                if sor_szam:
                    # Feltételezzük, hogy az End_Time az 5. oszlop (E oszlop) a táblázatban
