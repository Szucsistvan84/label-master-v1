...
def process_uploaded_pdfs(up_files, client, sheet_id, ugyfelkor_sheet_id, kivalasztott_datum):
    ...
    if all_rows:
        ...
        try:
            ...
            # Első feltöltésnél vagy frissítésnél golyóállóan lekérjük az eddig rögzített adatokat a felhőből a megőrzéshez!
            regi_beszedett_kp = 0
            regi_borravalo = 0
            try:
                ...
                if not summary_records_df.empty:
                    ...
                    for s_row in summary_records_df.to_dict('records'):
                        ...
                        if row_date == api_datum_kulcs and (summary_futar == futar_keresett_clean or summary_futar == "szűcs istván"):
                            ...
                            break
            except Exception as e_regi_read:
                st.write(f"⚠️ Információ: Nem sikerült a régi elszámolt értékek megőrzése: {e_regi_read}")
            ...
            szamitott_total_ertek = szamitott_kp_forgalom = regi_beszedett_kp = szamitott_borravalo = 0
        ...
        if not st.session_state.get('teszt_uzemmod', False):
            try:
                # Golyóállóan megőrizzük vagy nullázzuk az élő értékeket a PDF importáláskor
                uj_adat_sor = [
                    ...
                    int(szamitott_total_ertek), int(regi_beszedett_kp), int(szamitott_borravalo), int(szamitott_jutalek)
                ]
                ...
                st.cache_data.clear() # Cache törlése a sikeres szinkron után!
