...
def process_uploaded_pdfs(up_files, client, sheet_id, ugyfelkor_sheet_id, kivalasztott_datum):
    ...
    if meta_auto.get('datum_iso'):
        try:
            st.session_state['kivalasztott_datum'] = datetime.datetime.strptime(
                meta_auto['datum_iso'], "%Y-%m-%d"
            ).date()
    ...
    if all_rows:
        ...
        try:
            ...
            feltoltott_jaratok = [
                j for j in df_temp['Járat'].dropna().astype(str).str.strip().unique().tolist()
                if j != "" and j.lower() != 'nan'
            ] if 'Járat' in df_temp.columns else []
            ...
            ertek_col = next((
                ...
                if any(x in c.lower() for x in ['érték', 'ertek', 'forgalom', 'összeg', 'fizetendő', 'fizetendo', 'pénz', 'penz', 'összesen'])
            ), None)
            if ertek_col:
            ...
            api_datum_kulcs, aktualis_futar, jarat_szoveg, szamitott_osszes_megallo = (
                str(kivalasztott_datum), "Szűcs István", "Hiba", 0
            )
        ...
        try:
            ...
            fejlec = [
                "Datum", "Futar", "Jaratok", "Tervezett_Megallok", "Osszes_Cim",
                "Osszes_Etel", "Forgalom_Osszes", "Beszedett_KP", "Borravalo", "Vart_Jutalek"
            ]
            ws_summary = (
                sh_ugyfelkor.worksheet("Mobil_Summary")
                if "Mobil_Summary" in [w.title for w in sh_ugyfelkor.worksheets()]
                else sh_ugyfelkor.add_worksheet("Mobil_Summary", rows=500, cols=len(fejlec))
            )
        ...
        if not st.session_state.get('teszt_uzemmod', False):
            try:
                uj_adat_sor = [
                    api_datum_kulcs, aktualis_futar, jarat_szoveg, int(szamitott_osszes_megallo),
...
def render_mobil_sidebar_dashboard(client, SHEET_ID_UGYFELKOR):
    ...
    live_kesz_cimek = sum(
        1 for k in st.session_state.keys()
...
def render_desktop_sidebar_controls(client, SHEET_ID_MASTER, SHEET_ID_UGYFELKOR, LOG_FILE):
    ...
    if st.session_state.teszt_uzemmod:
        st.warning("⚠️ Adatbázis mentés letiltva!")
    st.divider()
    ...
    if is_admin:
        ...
        with col_w1:
            target_start_week = st.number_input(
                "Kezdő hét:", min_value=1, max_value=53,
                value=int(het_most) if het_most > 0 else 24, step=1,
                key="admin_sync_start_w"
            )
        with col_w2:
            target_end_week = st.number_input(
                "Záró (előre 3 hét):", min_value=1, max_value=53,
        ...
        with st.expander("👤 Felhasználó Kezelés"):
            ...
            edited_df_users = st.data_editor(
                df_to_edit, use_container_width=True, num_rows="dynamic", key="user_editor"
            )
...
def render_desktop_main_content(client, SHEET_ID_MASTER, SHEET_ID_UGYFELKOR, admin_funkcio, is_admin):
    ...
    if st.session_state.mdf is not None and not st.session_state.mdf.empty:
        ...
        if role == "futar" and 'user_jarat_lista' in st.session_state:
            df_view = df_view[
                df_view['Járat'].astype(str).isin([str(j) for j in st.session_state.user_jarat_lista])
            ].copy()
        ...
        if df_view.empty:
            ...
            jarat_nev = ", ".join(meta_m.get('jaratok', ['4002']))

            st.markdown(
                f"""
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px; margin-bottom: 4px; padding: 0 4px;">
                    <span style="font-size: 0.95rem; font-weight: 800; color: #139D43;">📊 Mai Műszerfal Összesítés</span>
                    <span style="font-size: 0.8rem; color: #6B7280; font-weight: 600;">Járat: {jarat_nev}</span>
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            with col_m3:
                st.metric("📦 Összes étel", f"{osszes_etel} adag")
            with col_m4:
                st.metric("💵 Rakományérték", f"{forgalom:,} {penznem}".replace(",", " "))
            with col_m5:
                st.metric("⭐ Várható Jutalék", f"{jutalek:,} {penznem}".replace(",", " "))
            ...
            with st.expander("🗺️ Útvonal megtekintése a térképen", expanded=False):
                ...
                with col_ast1:
                    ...
                    valasztott_ugyfel_str = st.selectbox(
                        "Melyik ügyfél koordinátáját javítod?", ugyfel_nevek, key="gps_assistant_selectbox"
                    )

                with col_ast2:
                    beillesztett_gps = st.text_input(
                        "Másolt koordináta beillesztése (Paste):",
            ...
            if st.button("🚀 DOKUMENTUMOK GENERÁLÁSA", type="primary", use_container_width=True, key="doc_gen_btn"):
                ...
                with st.spinner("⏳ PDF-ek generálása..."):
                    try:
                        ...
                        st.session_state['ready_manifest_pdf'] = create_manifest_pdf(
                            edited_df, st.session_state.c_n, st.session_state.c_p, meta
                        ).getvalue()
                        st.session_state['ready_raklista_pdf'] = create_raklista_pdf(
                            edited_df, aktualis_jaratok, meta, client.open_by_key(SHEET_ID_UGYFELKOR)
                        ).getvalue()
